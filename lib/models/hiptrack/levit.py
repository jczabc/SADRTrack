...
from lib.models.hiptrack.position_encoding import PositionEmbeddingSine

specification = {#C：表示模型不同阶段的通道数embed dim。D:整数，维度相关参数  N：代表不同层的头数 X：可能表示不同层块的重复次数
    'LeViT_128S': {
...
#num_classes：模型最后分类层的类别数.distillation：是否使用知识蒸馏技术 pretrained：是否加载预训练模型权重。fuse：是否融合卷积和批归一化层，用于加速推理（在训练和评估时可能有不同设置）。
# search_size：搜索区域的尺寸。template_size：模板的尺寸 template_number：模板的数量。neck_type：用于指定模型颈部（neck）的类型，用于特征融合或调整等操作。
# 函数内部调用 model_factory 并传入相应参数来创建模型实例。
@register_model
...
#FLOPS_COUNTER：全局变量，用于统计模型的浮点运算次数（FLOPs），初始化为 0，在模型构建过程中，不同层的计算会增加这个计数器的值，用于评估模型的计算复杂度。
class Conv2d_BN(torch.nn.Sequential):#组合卷积层和批归一化层。
    def __init__(self, a, b, ks=1, stride=1, pad=0, dilation=1,
                 groups=1, bn_weight_init=1, resolution=-10000):
        super().__init__()#a：输入通道数。b：输出通道数。ks：卷积核大小。stride：卷积步长。pad：填充大小。dilation：整数，空洞卷积的扩张率。
        # groups：整数，分组卷积的组数。bn_weight_init：批归一化层权重的初始化值 resolution：输入特征图的分辨率（用于计算 FLOPs）
...
#组合了线性层和批归一化层（针对 1 维数据）
#a：输入特征数量。b：输出特征数量。bn_weight_init：整数，批归一化层权重初始化值。resolution_x：整数，与搜索区域相关的分辨率。
#resolution_z：与模板相关的分辨率（用于计算 FLOPs）。template_number：整数，模板数量，默认值为 1。
...
#实现了注意力机制。
#dim：输入特征的维度。key_dim：键（key）的维度。num_heads：头的数量，用于多头注意力机制。
#attn_ratio：注意力计算中值（value）的维度与键维度的比例。activation：激活函数，用于后续层（未在初始化中完全使用，可能在模型构建时传入具体激活函数实例）。
#resolution_x：搜索区域的分辨率，默认值为 16。resolution_z：模板的分辨率=。template_number：模板数量
class Attention(torch.nn.Module):
    def __init__(self, dim, key_dim, num_heads=8,
        ...
        h = self.dh + nh_kd * 2#2*32*6+32*6*2 dim 384
        self.pos = "ori_pos"
        ...
        # dual-image position encoding
        else:#通过itertools.product生成一个二维平面上，在 x 坐标和 y 坐标都在 0 到 resolution_x - 1 范围内的所有坐标点组合，存储在 points 列表中
            points = list(itertools.product(range(resolution_x), range(resolution_x)))
            for i in range(template_number):#每次在原有坐标范围基础上按照 resolution_z 的步长扩展坐标范围来生成新的坐标点，并添加到 points 列表中
            ...
            N_xz = len(points)#总的坐标点数 N_xz
            attention_offsets_xz = {}#初始化一个空字典 attention_offsets_xz 用于存储不同的坐标点对之间的偏移量
            idxs_xz = []#存储每个坐标点对对应的偏移量索引。
            for p1 in points:
                for p2 in points:#计算每一对坐标点之间的偏移量 offset
                    offset = (abs(p1[0] - p2[0]), abs(p1[1] - p2[1]))
                    if offset not in attention_offsets_xz:
                        attention_offsets_xz[offset] = len(attention_offsets_xz)#该偏移量第一次出现（不在 attention_offsets_xz 字典中），则将其添加到字典中，并赋予一个新的索引（索引值为当前字典的长度
                    idxs_xz.append(attention_offsets_xz[offset])#将对应的偏移量索引添加到 idxs_xz 列表中
            self.attention_biases = torch.nn.Parameter(#创建了一个形状为 (num_heads, len(attention_offsets_xz)) 的可学习的参数张量 attention_biases，初始值全为 0
                torch.zeros(num_heads, len(attention_offsets_xz)))
            self.register_buffer('attention_bias_idxs',#注册了一个名为 attention_bias_idxs 的缓冲区张量
                                 torch.LongTensor(idxs_xz).view(N_xz, N_xz))#由之前计算得到的 idxs_xz 列表转换而来（转换为形状为 (N_xz, N_xz) 的 LongTensor 类型张量）
            # points = list(itertools.product(range(20), range(20)))
            #
            # N_xz = len(points)  # 总的坐标点数 N_xz
            # attention_offsets_xz = {}  # 初始化一个空字典 attention_offsets_xz 用于存储不同的坐标点对之间的偏移量
            # idxs_xz = []  # 存储每个坐标点对对应的偏移量索引。
            ...
            #         offset = (abs(p1[0] - p2[0]), abs(p1[1] - p2[1]))
            #         if offset not in attention_offsets_xz:
            #             attention_offsets_xz[offset] = len(
            #                 attention_offsets_xz)  # 该偏移量第一次出现（不在 attention_offsets_xz 字典中），则将其添加到字典中，并赋予一个新的索引（索引值为当前字典的长度
            #         idxs_xz.append(attention_offsets_xz[offset])  # 将对应的偏移量索引添加到 idxs_xz 列表中
            # self.attention_biases = torch.nn.Parameter(
            #     # 创建了一个形状为 (num_heads, len(attention_offsets_xz)) 的可学习的参数张量 attention_biases，初始值全为 0
            #     torch.zeros(num_heads, len(attention_offsets_xz)))
            # self.register_buffer('attention_bias_idxs',  # 注册了一个名为 attention_bias_idxs 的缓冲区张量
            #                      torch.LongTensor(idxs_xz).view(N_xz,
            #                                                     N_xz))  # 由之前计算得到的 idxs_xz 列表转换而来（转换为形状为 (N_xz, N_xz) 的 LongTensor 类型张量）
        global FLOPS_COUNTER
        #queries * keys
    ...
    def forward(self, x):  # x (B,N,C)
        if self.pos == "absolute_pos":
            ...
            k = k.permute(0, 2, 1, 3)
            v = v.permute(0, 2, 1, 3)
            ab1=self.attention_biases[:, self.attention_bias_idxs]
            at1= (q @ k.transpose(-2, -1)) * self.scale
            if self.pos == "ori_pos" or self.pos == "s_center" or self.pos == "no_crash" or self.pos == "s&&t" or self.pos =="s&&s_t&&t_s&&t" or self.pos == "s_t_line" or self.pos == "s_t_row":
                attn = (
                    (q @ k.transpose(-2, -1)) * self.scale
                    +
        ...
        return x
#stride：下采样步长。resolution_x：搜索区域的分辨率。resolution_z：模板的分辨率。template_number：模板数量。
class Subsample(torch.nn.Module):
    ...
    def forward(self, xz):
        ...
        return xz_
#in_dim：输入维度。out_dim：输出维度。num_heads：头的数量。attn_ratio：注意力计算中值的维度与键维度的比例。
#activation：激活函数，用于后续层。stride：下采样步长。resolution_x：原始搜索区域分辨率。resolution_z：原始模板分辨率。
#resolution_x_：下采样后的搜索区域分辨率。resolution_z_：下采样后的模板分辨率。template_number：模板数量。
...
class LeViT(torch.nn.Module):
    ...
    """
    #search_size：搜索区域的尺寸。template_size：模板的尺寸。template_number：模板数量 patch_size：补丁（patch）的大小，用于将输入图像划分成小块。
#neck_type：字符串，模型颈部类型，默认值为 'LINEAR'。in_chans：输入图像的通道数，默认值为 3（例如 RGB 图像）。num_classes：分类的类别数。
#embed_dim：模型不同阶段的嵌入维度。
#key_dim：列表，包含整数，键的维度相关参数，默认值为 [64]。
#depth：列表，包含整数，模型不同层块的深度（重复次数），默认值为 [12]。
#num_heads：列表，包含整数，多头注意力机制的头数，默认值为 [3]。
#attn_ratio：注意力机制中值维度与键维度的比例，用于控制注意力层的输出维度。mlp_ratio：多层感知机（MLP）中间层维度与输入维度的比例，用于控制 MLP 层的维度扩展。
#hybrid_backbone：可以是一个自定义的混合卷积神经网络（CNN）骨干网络，用于对输入图像进行初步特征提取，如果为None，则使用默认的b16函数构建的骨干网络。
#down_ops：列表，包含一些操作的配置信息，用于下采样和调整模型结构，例如['Subsample', D, embed_dim[0] // D, 4, 2, 2]表示进行下采样操作，其中包含下采样相关的参数（如维度、头数、比例等）以及步长信息。
#attention_activation：激活函数，用于注意力机制中的激活操作，默认值为torch.nn.Hardswish。
#mlp_activation：激活函数，用于多层感知机中的激活操作，默认值为torch.nn.Hardswish。
#distillation：布尔值，是否使用知识蒸馏技术，默认值为True。#drop_path：随机失活路径的概率，用于防止过拟合，默认值为0。
    def __init__(self,
                 search_size=384,
                 template_size=384,
                 template_number=1,
...
def model_factory(C, D, X, N, drop_path, weights,
    ...
    if pretrained:
        print('Load pretrained model from:  +levit')
