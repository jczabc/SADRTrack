...
from lib.models.hiptrack.position_encoding import PositionEmbeddingLearned
...
specification = {
    ...
    'LeViT_192': {
        'C': '576_288_384', 'D': 32, 'N': '3_5_6', 'X': '4_4_4', 'drop_path': 0,
...
def LeViT_384(num_classes=1000, distillation=True,
              pretrained=False, fuse=False,
              search_size=384, template_size=112, template_number=1, neck_type='LINEAR'):
...
class Attention(torch.nn.Module):
        ...
        if self.pos != "ori_pos":
            ...
            ##### avoid the crash#######
               ...
               # resolution_all = resolution_z + resolution_x
                resolution_all = resolution_x
                self.pos_lea = PositionEmbeddingLearned(num_pos_feats=self.dim,normalize=True,shape=resolution_all)
            elif self.pos == "single_image":
                # Single-image position encoding logic
                points = list(itertools.product(range(resolution_x), range(resolution_x)))
                N_xz = len(points)
    ...
    def train(self, mode=True):
        ...
        if mode and hasattr(self, 'ab'):
            ...
            if self.pos == "ori_pos" or self.pos== "single_image" or self.pos == "s_center" or self.pos == "no_crash" or self.pos == "s&&t" or self.pos =="s&&s_t&&t_s&&t" or self.pos == "s_t_line" or self.pos == "s_t_row":
    ...
    def forward(self, x):  # x (B,N,C)
        if self.pos == "absolute_pos":
            ...
            pos = pos.reshape(4, 320, 576)
            q = self.q(x+pos).view(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            k = self.k(x + pos).view(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            q = self.q(x).view(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
            k = self.k(x).view(B, N, self.num_heads, -1).permute(0, 2, 1, 3)
