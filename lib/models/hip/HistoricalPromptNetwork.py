...
from lib.models.hiptrack import levit as levit_module
...
class HistoricalPromptDecoder(nn.Module):
    ...
    def readout(self, affinity, mv, qv, eval=False):
        if not eval:#torch.Size([4, 384, 24, 24])
            mv = mv.flatten(start_dim=2)#torch.Size([4, 384, 576])
        B, CV, THW = mv.shape
        _, _, H, W = qv.shape
...
class HistoricalPromptNetwork(nn.Module):
    def __init__(self):
        ...
        levit_model = getattr(levit_module, 'LeViT_384')(
            num_classes=0,
            distillation=False,
            pretrained=True,
            fuse=False,
        ...
        levit_model1 = LeViT(num_classes=0,
                            distillation=False,
                            neck_type='FB',
                            embed_dim=embed_dim,
                            depth=[4, 4, 4],
        ...
        self.encoder = HistoricalPromptEncoder(levit_model=levit_model)
    ...
    def encode(self, frame,sf, kf16, mask, other_mask=None):
        f16 = self.encoder(frame,sf, kf16, mask)
    ...
    def eval_decode(self, queryFrame_key, queryFrame_value):
        ...
        return self.decoder.readout(affinity=None, mv=self.decoder.mem_v, qv=queryFrame_value, eval=True)
    ...
    def decode(self, queryFrame_key, queryFrame_value, memory_key, memory_value):
        ...
        return self.decoder.readout(affinity=None, mv=memory_value, qv=queryFrame_value)
