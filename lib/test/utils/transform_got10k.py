...
def transform_got10k(tracker_name, cfg_name,epoch_num):
    #epoch_num = str(epoch_num).zfill(4)  # 例如 20 → '0020'
    ...
    # make zip archive
    # make zip archive with epoch number in name
    zip_src_name = os.path.join(result_dir, f"{tracker_name}_{cfg_name}_got10k_src_ep{epoch_num}")
    zip_dest_name = os.path.join(result_dir, f"{tracker_name}_{cfg_name}_got10k_submit_ep{epoch_num}")

    shutil.make_archive(zip_src_name, "zip", src_dir)
    shutil.make_archive(zip_dest_name, "zip", dest_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--epoch_num', type=str, help='epoch num')

    args = parser.parse_args()
    transform_got10k(args.tracker_name, args.cfg_name,args.epoch_num)
