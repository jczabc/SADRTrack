...
> Note: If your code still uses the original script/config names, replace `sadrtrack` and `sadrtrack_got` with the actual names in `lib/train` and `experiments`.
...
To compute FLOPs and parameters, run:

```bash
bash tracking/profile_sadrtrack.sh
```

If the profiling script has not been renamed, use the existing profiling script and update the tracker/config name inside it.
...
If you find this work useful for your research, please consider citing SADRTrack:

```bibtex
@article{ji2026sadrtrack,
  title={SADRTrack: Self-Evolving Template via Static Anchor and Dynamic Refinement for Visual Tracking},
  author={Ji, Cuize and Li, Hui and Hu, Wentao and Yang, Kai and Wei, Xiong},
  journal={Manuscript},
  year={2026}
}
```
...
For questions about this project, please create an issue or contact the authors.
